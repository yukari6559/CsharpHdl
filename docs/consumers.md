...
書き方の参考: リポジトリの `examples/`（製品 ISA は含みません）。`Sim` は **S2 完了**（Mem3種・Instance・Load/Peek/Poke・AdvanceWhile）。いまの本線は MidiSynth **M1b If/Else**（M1a 比較は `0.3.6` 済み。内部 ROADMAP / tickets 参照）。
