...
| T2b | ビットスライス / 連結 | 命令デコードでフィールドが取れる | **完了**（`DecodeSliceConcat` テスト） |
| T2c | 2 読み 1 書きレジスタファイル（または同等の Mem／ポート） | x0〜x31 相当が書ける | **完了**（`RegFile2R1W` テスト） |
| T2d | バイト／部分書き込み | LB/SB 等のメモリが書ける | **完了**（`ByteWriteRam` テスト） |
| T2e | 符号拡張 | 即値・ロードの符号拡張が書ける | **完了**（`ExtendPass` テスト） |
